#include "main.h"
/**
 * main - a main function make the system call, create process, request services
 * @ac: number of argument
 * @av: a array to store each argument content
 *
 *  Return: 0 on success
 */
int main(int ac, char **av)
{
	...
	while (rd_stat)
	{
		rd_stat = read(fd_from, buffer, BUFFER_SIZE);
		if (rd_stat == -1)
		{
			dprintf(STDERR_FILENO, "Error: Can't read file %s\n", av[1]);
			exit(98);
